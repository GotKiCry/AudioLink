...
- 接收未连接时只展示连接表单与同一局域网的引导，不展示空遥测和禁用控件；连接后先显示真实状态、来源和断开操作，再显示设备列表。「连接另一台主机」按需展开，地址输入与连接动作始终可见。
- **音量只有一处**：App 不设总音量控件 —— 总响度归系统音量键（本机播放走 `USAGE_MEDIA`，本来就被媒体流音量控制）；界面只提供设备通道里每台主机**这一路**的本地增益（0–200%，仅影响本机混音，不出网）。
- 发送视图直接提供本机地址、等待接入、采集源和推流控制，不再把正常功能放入诊断。连接错误在两个视图均可见。
...
### Desktop 声音共享工作区（2026-09-21）

桌面用户在家中电脑前完成连接，随后把窗口留在后台；保留 Fluent 深浅主题与蓝色强调色，用稳定的布局区分「等待连接」「推流中」「已暂停」。

- 左侧固定音源与共享控制，暂停／恢复紧邻状态；右侧依次为页面说明、连接地址与连接步骤、设备列表、接收其他主机声音的折叠入口。
- 设备接入后默认自动推流；读取自动推流偏好、音源就绪前不抢跑。暂停对整个共享会话生效，新设备或重连也不能取消暂停；用户恢复后才重新发送。
- 等待设备时不称「推流中」，启动失败不伪装成等待。暂停期间保持连接，允许切换音源。桌面外壳目前单路发送，其他已连接设备等待当前推流结束，不并发提交必然失败的启动请求。
- 页面标题 28/36，区域标题 16/24，正文 14/20，辅助文案 12/18。保留系统字体、4px 控件和 8px 面板圆角；内容宽度最大 1120px，1024×640 窗口内部滚动。
- 常驻设备列表用实体表面、分隔线和文字状态；设备指纹、协商能力按需展开。连接步骤只描述用户动作，不出现开发里程碑、内部状态机或设计说明。
- 推流用绿色、等待和暂停用中性色，告警和失败分别用黄、红。新区域不使用装饰性玻璃、侧条、宽投影；背景与浮层继续复用原有配置和 portal。
- 浅色三级文字统一为 #616166，深色为 #ABADB6，提高自定义背景上辅助信息的可读性。
...
| `onPrimaryContainer` | `#9CD3FF` | `#0F548C` | brandWeb 浅档 / brandWeb[80]。**深色按实际配对取 4.94:1** —— 原值 `#62ABF5` 仅 3.25:1（当时连接卡片与省电白名单卡的正文），见「配对法则」 |
...
接收端先显示「局域网主机」列表，再提供手动地址输入。条目只展示名称、地址和连接动作，已连接与版本不兼容有明确禁用状态。扫描失败时保留手动入口并自动重试；空列表说明同网段与主机开启条件。连接动作保持突出，不用发现列表分散注意力。

桌面将发现列表放在「收听其他设备的声音」折叠区内；主机指引优先提示在接收端选择电脑。Android 在前台且接收入口可见时扫描，折叠入口与后台阶段释放发现资源；现有播放不受影响。颜色、字体与按钮沿用现有 Fluent 令牌。


### 音频链路读数（2026-09-21）

桌面设备卡片在连接后直接提供网络延迟（RTT）、音频丢包率、累计欠载与当前缓冲水位，使用现有 caption / numeral 档与语义色，允许换行。丢包非零以文字、数字与警示色共同表示。窄窗口保留工具栏读数并换行，不隐藏它们。Android 接收面板沿用原有 Readout，延迟明确标为网络 RTT，丢包率保留两位小数。

RTT 是实测网络往返延迟，不冒充端到端音频延迟。没有有效接收端反馈时显示「—」和等待文案；有效的 0.00% 表示最新一秒内恢复后没有确认缺包。欠载与丢包分列，避免丢包为零时误导用户认为不会断音。详细口径见 docs/67-audio-link-quality.md。
