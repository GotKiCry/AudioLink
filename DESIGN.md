...
- 左侧固定音源与共享控制，暂停／恢复紧邻状态；右侧依次为页面说明、连接地址与配对步骤、设备列表、接收其他主机声音的折叠入口。
- 设备完成配对并连接后默认自动推流；读取自动推流偏好、音源就绪前不抢跑。暂停对整个共享会话生效，新设备或重连也不能取消暂停；用户恢复后才重新发送。
...
接收端先显示「局域网主机」列表，再提供手动地址输入。条目只展示名称、地址和连接动作，已连接与版本不兼容有明确禁用状态。扫描失败时保留手动入口并自动重试；空列表说明同网段与主机开启条件。配对阶段继续突出 PIN 输入，不用发现列表分散注意力。

桌面将发现列表放在「收听其他设备的声音」折叠区内；主机指引优先提示在接收端选择电脑。Android 在前台且接收入口可见时扫描，配对、折叠入口和后台阶段释放发现资源；现有播放不受影响。颜色、字体与按钮沿用现有 Fluent 令牌。
