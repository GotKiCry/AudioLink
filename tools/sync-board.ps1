...
$Tasks = @(
    ...
    @{ T = '[M5] 启动后自动连接上次设备（FR-31 剩余部分）'; M = 'M5'; P = 'P2'; S = 'Todo'; B = '2026-09-16 立（lib.rs 里剩下的那条 TODO(M5)）。FR-31 的"自动连接上次设备（可关）"还没做：现在每次启动都要用户手动填 IP 或等发现。要做的：① 记住上次成功连接的设备（store 插件已注册）；② 启动时按开关自动重连（默认关？—— 需要决定默认值，安全侧是先不自动连）；③ 与托盘/常驻行为配合（常驻 + 自启 + 自动重连才是"开机就能用"）。' }
