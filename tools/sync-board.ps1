...
$Tasks = @(
    ...
    @{ T = '[M5] Android release 打包流水线（assembleRelease + keystore 走 CI secrets）'; M = 'M5'; P = 'P1'; S = 'Todo'; B = '盘点发现（docs/42 §2.4）：release 签名曾人工打通（docs/12 §8.5：keystore + R8 规则 + gitignore 修口），但 CI 里只有 assembleDebug。发布要的是 assembleRelease + 从 CI secrets 注入 keystore（不能把密钥提交进仓库）。同时要确认双 ABI（arm64-v8a / armeabi-v7a）在 release 下都产出且都能装。' }
