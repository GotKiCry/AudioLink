...
$Tasks = @(
    ...
    @{ T = '[M1] 真机验收：Android 低延迟/出声延迟/30 min 无断流'; M = 'M1'; P = 'P1'; S = 'Todo'; B = '**阻塞在硬件**：本机 adb 无设备、无可用 AVD。执行顺序（按这个顺序能把故障面切开）：① **先点 UI 上的「内核自检」** → protocolSelfTest() 跑 §12 三组 golden vectors 并与 audiolink-proto 的测试文件逐字节比对，过了就说明内核在真机上活着、JNA 加载 .so 成功、ABI 装对了 —— 于是后面若连不上，只剩网络/配对可查；② 启服务（引擎随服务启停，监听内核默认端口 58290）；③ PC 端 connect + 提交 PIN；④ 看 getPerformanceMode() 实测返回值（UI 显示原值，不做粉饰）、出声延迟、30 min 连续无断流与 underruns/plc_count。Android 侧代码与 43 个 JVM 用例已就绪，接上设备即可复跑' }
