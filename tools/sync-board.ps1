...
$Tasks = @(
    ...
    @{ T = '[M5] 自动更新密钥对（updater pubkey 目前是占位符，功能不可用）'; M = 'M5'; P = 'P1'; S = 'Todo'; B = '盘点发现（docs/42 §2.1）：tauri.conf.json 的 plugins.updater.pubkey 现在是 REPLACE_WITH_TAURI_UPDATER_PUBKEY，createUpdaterArtifacts 也是 false。也就是说「配置看起来都在，功能实际不可用」—— 没有密钥对就签不出更新包。要做的：① 用 tauri signer 生成密钥对；② 私钥进 CI secrets、公钥写进 tauri.conf.json；③ 把 createUpdaterArtifacts 打开；④ 真机验证一次「旧版本 → 检测到更新 → 下载 → 安装」。这条最容易被人以为「已经配好了」，所以单独列出来。' }
