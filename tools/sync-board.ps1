...
$Tasks = @(
    ...
    @{ T = '[M5] Android 依赖的合规材料（Gradle 依赖进清单 + 声明投放）'; M = 'M5'; P = 'P2'; S = 'Todo'; B = '盘点发现（docs/42 §2.5）：许可审计只覆盖 Rust 与前端（docs/41 §5 的覆盖面清单里已明记），Gradle 依赖还没进清单，Android 侧也没有声明的投放位置。要做的：① 用 Gradle 的依赖报告（或 license 插件）采一份依赖清单；② 并入 license-audit 的判定与报告；③ 在 Android 应用内提供一个「开源许可」入口（类似桌面端的关于面板）。' }
