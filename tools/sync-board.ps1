...
$Tasks = @(
    ...
    @{ T = '[M5] Android 侧声明投放（应用内「开源许可」入口）'; M = 'M5'; P = 'P2'; S = 'Todo'; B = '2026-09-16 从原项拆出（另一半：Gradle 依赖进清单已完成，见相邻的 Done 行）。桌面端已有「关于」面板读声明（docs/41 §7），Android 应用内**还没有对应入口**。要做的：① 把 THIRD-PARTY-NOTICES 打成 Android 资源（assets/）；② 应用内加一个「开源许可」页（Compose）读它并展示；③ 与桌面一致：找不到时说清怎么生成，而不是空白页。另：Android 许可审计目前是「发布前手动跑」，还没进 CI（桌面侧在 desktop job 里）—— 是否值得加进 android job 可以一并决定。' }
