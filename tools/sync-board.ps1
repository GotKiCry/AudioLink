...
$Tasks = @(
    ...
    @{ T = '[M4] 能力协商（界面侧）：引擎查询 API + 能力展示 + 不可用置灰'; M = 'M4'; P = 'P2'; S = 'Todo'; B = '本轮只做了内核侧：协商结果存在 Handshake::agreed_caps() 里，但还没接到 Engine 的查询 API 与事件上 —— 所以桌面端现在看不到「协商出了什么」，也不知道内录/麦克风这类可选能力对端有没有。要做：① Engine::capabilities(peer) 之类的查询 + 一个 EngineEvent（含双方位图与交集）；② 桌面端对端卡片或设置页展示能力标记；③ 内录/麦克风不可用时把入口置灰并说明原因（M4 交付物 4 的「降级」那一半）。注意 ③ 依赖平台能力真的接上（Windows WASAPI loopback 已有、Android AudioPlaybackCapture 尚未实现），所以先做 ①②。' }
