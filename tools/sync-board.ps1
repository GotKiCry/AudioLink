...
$Tasks = @(
    ...
    @{ T = '[CI] core-heavy 的 229 s 测试步骤：27 个测试二进制链接是主体（engine 一个 crate 就 13 个）'; M = 'M0'; P = 'P3'; S = 'Done'; B = '2026-09-16 完成（先量再改，按这条待办自己写明的要求）。**先量**：清掉引擎产物重编，15 个可执行文件（1 lib + 14 集成测试）共 **7.4 s**，平均每个约 0.5 s → 合并 14 → 1 理论上省 13 次「编译 + 链接」。**两个踩坑**：① crate root 的 mod 解析基准是**它自己所在的目录** —— tests/engine.rs 里的 mod x; 找的是 tests/x.rs 而不是 tests/engine/x.rs；文件还留在 tests/ 时这条回退路径**悄悄生效**，结果独立二进制与 harness 同时构建、同一批测试跑两遍（nextest 报 182 而不是 163）。② 正确布局是 **tests/engine/main.rs 作 root、14 个文件放在它旁边**（目录里的 main.rs 也是合法测试目标，且模块基准就是那个目录）。**结果**：测试二进制 15 → **2**；本地重编 7.4 → 6.3 s；**CI 的 core-heavy test 步骤 187–200 s → 166 s**；测试总数 437 → **437（一个没少）**。单跑仍可以：cargo nextest run -p audiolink-engine --test engine -E "test(group_epoch)"。**还剩**：其他 crate 同样结构（proto 4 个、tools 4 个且依赖 engine + QUIC 链接更贵），同样手法还能再压 —— 但收益随数量下降，值不值要再量。详见 docs/26-ci-key-path.md §10。' }
