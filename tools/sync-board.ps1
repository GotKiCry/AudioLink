...
$Tasks = @(
    ...
    @{ T = '[M1] 验收：P50 ≤ 110 ms / P95 ≤ 150 ms、零重采样、低延迟模式生效'; M = 'M1'; P = 'P1'; S = 'In Progress'; B = '2026-09-16 更新：PHK110 / Android 16 / API 36 最新包已安装并回拉比对，PIN 和普通/快速/推流中/配对中启停已验收；LOW_LATENCY、48 kHz / 2ch、PCM_FLOAT 与 FAST 路径生效。180 s 合成基线全程 streaming，但 Rust 待播队列 40→300 ms（P50 240 ms），欠载 0→15、迟到 0→1；手机 PCM 环约 100 s 区间溢出/读空无新增。独立队列增长仍需修复；整体延迟及修复后长跑未完成，不能标 Done。详见 docs/16-android-service-lifecycle.md。 历史记录：PC↔PC 段：link-loop 探针 P50 20.1 / P95 20.6 ms。**PC→Android 段已实测（2026-09-15，真机 MI 8 Lite）**：零重采样 ✅（全链路 48000 Hz、flinger 原值 PCM_FLOAT + PRIMARY|FAST）、低延迟模式 ✅（getPerformanceMode()=LOW_LATENCY）、四轮推流零断流、丢包末值 0.00%。**延迟未达标**：e2e 下限 ≥115 ms（真实 WASAPI 采集，不含设备输出），加设备侧 80–101 ms ≈ 195 ms；根因两条——(a) 内核→Kotlin PCM 推送 ≈2× 实时导致接收侧持续积压，(b) 设备 AudioTrack 默认容量 3844 帧/80 ms。详见 docs/12-m1-device-acceptance.md' }
    ...
    @{ T = '[M1 听感缺陷] 接收侧待播队列持续增长，触顶后丢帧'; M = 'M1'; P = 'P0'; S = 'In Progress'; B = '2026-09-16 PHK110 / API 36 新包复测已确认独立缺陷。180 s 合成 440 Hz / 20 ms 帧：Rust 待播队列首值 40 ms、P50 240 ms、末值 300 ms；内核欠载 0→15、迟到丢弃 0→1，全程 streaming。手机 PCM 环同一约 100 s 区间溢出和读空均无新增，设备无新增静音/丢弃/写错，PCM 双倍供给未复现。队列在欠载附近阶跃增加，不能沿用恒定 5500 ppm 漂移解释；需核实播放时钟已推进后迟到帧仍被播放的路径。已有证据 docs/16-android-service-lifecycle.md、target/evidence/phk110-validation/baseline-180.json/log。下一步修复并复测，M1 延迟与长跑仍未达标。' }
    ...
    # ---- M2–M5 ----
    @{ T = '[M2] 稳定性与质量：抖动缓冲 + 双发/PLC/NACK + 自适应码率 + 遥测面板'; M = 'M2'; P = 'P2'; S = 'In Progress'; B = 'docs/05-roadmap.md M2；验收含 8 h soak 与弱网（5 Mbps / 2% 丢包 / 30 ms 抖动）' }
    @{ T = '[M2] 自建丢包掩盖（CELT-only 无真 PLC：重复上一包 + 淡出 + 交叉淡化）'; M = 'M2'; P = 'P1'; S = 'In Progress'; B = '实测（2026-09-14）：opus-rs 在 CELT-only 下第 2 个丢失帧起硬静音、恢复有 -45 dB 凹陷，「PLC 兜底」不成立 → 必须自建；见 ADR-003 注记' }
