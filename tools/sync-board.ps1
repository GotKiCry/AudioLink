...
$Tasks = @(
    ...
    @{ T = '[M4] 能力协商（降级）：依赖能力的入口按协商结果置灰'; M = 'M4'; P = 'P2'; S = 'Todo'; B = '本轮只做到「说明」：对端卡片会写清本端有而对端没有的能力（missingOnPeer），但依赖该能力的入口还没有真的禁用。要做：① 建组勾选列表里把不支持 GROUP_EPOCH 的对端标为不可选并说明；② 其余依赖能力的入口（混音路数、无损档 PCM16）同样处理；③ 把「哪些入口依赖哪些能力」在界面上说清楚，否则置灰会变成「点了没反应」。后半条依赖平台能力注入（见内核侧条目的说明）。' }
