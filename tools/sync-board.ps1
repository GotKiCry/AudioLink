...
$Tasks = @(
    ...
    @{ T = '[CI] core-heavy 的 229 s 测试步骤：27 个测试二进制链接是主体（engine 一个 crate 就 13 个）'; M = 'M0'; P = 'P3'; S = 'Todo'; B = '实测（run 35098695833）：core-heavy 的 test 步骤 229 s，而真正的测试运行只有约 40 s（各 suite 的 finished in 之和），其余是 27 个测试二进制的编译与链接 —— audiolink-engine 一个 crate 就贡献 13 个集成测试（adaptive_bitrate / clock_sync / engine_shutdown / gain_control / group_epoch / group_management / handshake_deadline / mixer_alignment / mixer_convergence / multi_session / nack_retransmit / pcm_delivery / playout_recovery），每个都要在 Windows 上把整棵 engine + QUIC 栈链接一遍。两条候选路径：① 把 engine 的集成测试合并进一个 harness（tests/all.rs + tests/all/*.rs 子模块，过滤照样可用 cargo test --test all <name>）；② 按依赖再拆 job（audio+net 与 engine+ffi+tools 并行）。动手前先量合并方案的收益 —— 本地热缓存下 test heavy 只要 73 s，说明这层开销对缓存与平台都敏感。' }
)
