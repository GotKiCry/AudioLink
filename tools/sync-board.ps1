...
$Tasks = @(
    ...
    @{ T = '[M5] 发布流程：tag → 构建 → GitHub Release（Release notes + 校验和）'; M = 'M5'; P = 'P1'; S = 'Todo'; B = '盘点发现（docs/42 §2.3）：目前 CI 只在 push/PR 上跑检查，**没有任何 job 产出可分发的产物** —— Android 的 debug APK 是构建验证，不是发布物。缺的是：tag 触发的 release job、产物命名规范（版本 + 平台 + 架构）、Release notes 模板、SHA-256 校验和。**这一步做完，M5 才真的走到「能发给别人」。** 依赖：桌面声明必须先由 -Notices 生成（bundle.resources 收了它，没生成就打不出包）。' }
