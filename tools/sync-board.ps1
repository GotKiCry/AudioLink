...
$Tasks = @(
    ...
    # ---- M2–M5 ----
    @{ T = '[M2] 稳定性与质量：抖动缓冲 + 双发/PLC/NACK + 自适应码率 + 遥测面板'; M = 'M2'; P = 'P2'; S = 'In Progress'; B = 'docs/05-roadmap.md M2；验收含 8 h soak 与弱网（5 Mbps / 2% 丢包 / 30 ms 抖动）。2026-09-16：播放游标、PCM 丢包掩盖、自适应抖动深度与有界重排已完成。当前按 P95 抖动/欠载在 20/40/60 ms 三档调整，升档按实际水位缺口补齐，稳定 30 s 后逐档下降；60 ms 档等待一帧修复乱序。PHK110 90 s 队列 P50/P95/max 均 60 ms、链路丢包/PLC 0，欠载/迟到 52/51 且最后 21 s 无新增，AudioTrack underrun 0、FastMixer writeErrors 0。下一步推进冗余双发/NACK、码率自适应、真实弱网和 8 h soak。详见 docs/17-m2-pcm-concealment.md、docs/18-m2-adaptive-jitter.md。' }
    @{ T = '[M2] 自适应抖动缓冲（20–60 ms）+ 有界乱序重排'; M = 'M2'; P = 'P1'; S = 'Done'; B = '2026-09-16 完成。接收侧以 1 Hz 到达抖动 P95 和新增欠载驱动 20/40/60 ms 三档目标：升档立即，连续稳定 30 s 后逐档下降；播放线程按实际水位缺口执行最多 60 ms 的有界重缓冲，降档只丢真实多余深度，且原子发布不会覆盖并发欠载升档。仅 60 ms 档允许等待一帧修复乱序，待排序包上限 3；超时转 PCM 掩盖，覆盖重复、迟到、回绕和大跳变。真实 Opus 回归验证 0/2/1 修复与超时 PLC；引擎 95 单测 + 10 集成、Clippy 通过。PHK110 90 s 零链路丢包/PLC，队列 P50/P95/max 均 60 ms，欠载/迟到 52/51 且最后 21 s 无新增，AudioTrack underrun 0、FastMixer writeErrors 0。真实 2%/30 ms 弱网及 8 h soak 留在 M2 父任务。详见 docs/18-m2-adaptive-jitter.md。' }
