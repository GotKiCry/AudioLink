...
$Tasks = @(
    ...
    @{ T = '[桌面] 音量滑块（set_peer_gain 已通到 IPC，UI 未接）'; M = 'M3'; P = 'P2'; S = 'Todo'; B = '引擎侧 SET_GAIN 已生效（docs/34）、桌面桥接与 api/hook 动作已就绪（docs/35），缺的只是 UI：对端卡片上放一个 range 滑块（0–200%）。注意对端卡片当前把「停止推流 / 开始推流 / 等待配对」放在一个三元里，加滑块要包一层 fragment 或独立渲染，别把三元的并列结构搞坏。' }
