...
$Tasks = @(
    ...
    @{ T = '[M4] 多源共同时间基准（接收端广播 epoch + 发送端预约发送）'; M = 'M4'; P = 'P2'; S = 'Todo'; B = '协议 §7 的 epoch 是「发送端指定、接收端排播」，于是同一发送端 → 多台接收端能对齐；但 M4 的拓扑（2 部手机 → 1 台 PC）里两个发送端是彼此独立的源，**当前实现下它们之间没有共同时间基准**，也就做不到采样级对齐。需要两件新东西：① 接收端（PC）作为基准来源，广播 epoch 给所有发送端（协议方向需要扩展，现在只能发送端指定）；② 发送端的「预约发送」—— 把 epoch 用在自己的发送时间轴上（sample_index 的 0 点对齐到 epoch），而不是从启流瞬间开始计数。现状与实测见 docs/38-m4-mixer-alignment.md §2。' }
