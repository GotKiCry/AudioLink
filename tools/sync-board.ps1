...
$Tasks = @(
    ...
    # ---- M2–M5 ----
    @{ T = '[M2] 稳定性与质量：抖动缓冲 + 双发/PLC/NACK + 自适应码率 + 遥测面板'; M = 'M2'; P = 'P2'; S = 'In Progress'; B = 'docs/05-roadmap.md M2；验收含 8 h soak 与弱网（5 Mbps / 2% 丢包 / 30 ms 抖动）。2026-09-16：播放游标、PCM 丢包掩盖、自适应抖动/有界重排和默认冗余双发已完成。副本延迟一帧，接收按序号去重且质量指标只取首个有效副本；最高 60 ms 档欠载可同档补水。PC 双 Engine 与 PHK110 真实 QUIC 码率均稳定 320.8 kbps；手机 90 s 队列 P50/P95/max 60 ms，链路丢包/PLC 0，欠载/迟到 24/22 且第 75–90 s 无新增，AudioTrack underrun 0、FastMixer writeErrors 0。NACK 重传、soak-runner 与自适应码率（§8 的码率档）均已落地（docs/21、docs/22、docs/23）。下一步只剩：真实弱网（5 Mbps / 2% 丢包 / 30 ms 抖动）下的降级实测、遥测面板，以及**真正跑一次 8 h soak**；单声道降级与帧长 10 ms 切换已在 LinkSeverity 记账、留待专门任务。详见 docs/17-m2-pcm-concealment.md、docs/18-m2-adaptive-jitter.md、docs/19-m2-redundant-send.md。' }
