...
$Tasks = @(
    ...
    @{ T = '[M3] 组控制帧（GROUP_CREATE / JOIN / LEAVE / EPOCH）载荷与接线'; M = 'M3'; P = 'P2'; S = 'Todo'; B = '0x43 GROUP_EPOCH 已于 2026-09-16 接线完成（见另一条 M3 记录与 docs/29）。本条只剩组管理三帧：audiolink-engine::payload 增加 GroupCreate/GroupJoin/GroupLeave 载荷（postcard，成员身份用 NodeId 的指纹字节）；dispatch 分支接线；组内成员表与动态加入/退出、成员同步质量展示（M3 交付物 4）。' }
