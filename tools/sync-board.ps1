...
$Tasks = @(
    ...
    @{ T = '[CI] core-heavy 的剩余杠杆：153 s 编译里 27 个测试 target 的重复编译'; M = 'M0'; P = 'P3'; S = 'Todo'; B = '实测（run 35101117735）：core-heavy 的 test 步骤 199 s 里约 153 s 是编译 9 个本地 crate 与 27 个测试 target。本地 crate 必须重编（每次 push 都是新代码），但测试 target 的重复是结构性的：engine 一个 crate 就有 13 个集成测试二进制，每个都要独立编译一遍文件再链接一次。候选方案：把 13 个 tests/*.rs 移进 tests/all/ 子目录、用 tests/all.rs 以 mod 引入（各文件内容原样不动，#![allow] 在 mod 内合法；过滤仍可用 cargo nextest run --test all -E test(group_epoch)）。预期收益 20-30 s 级，**动手前必须先量**：本轮已经因为一个未验证的前提（链接是主体）在看板上写过错结论，不要再犯第二次。' }
)
