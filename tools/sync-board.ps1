...
$Tasks = @(
    ...
    # ---- M2–M5 ----
    @{ T = '[M2] 稳定性与质量：抖动缓冲 + 双发/PLC/NACK + 自适应码率 + 遥测面板'; M = 'M2'; P = 'P2'; S = 'In Progress'; B = 'docs/05-roadmap.md M2；验收含 8 h soak 与弱网（5 Mbps / 2% 丢包 / 30 ms 抖动）。2026-09-16 PHK110：M1 播放游标修复已阻止迟到帧累积延迟，180 s 待播队列 P50/P95/max 稳定在 40/40/60 ms，但欠载/迟到各新增 143 次（链路丢包 0%，手机 PCM 环无新增读空/溢出）。下一步用有界重排/自适应目标深度吸收到达与调度抖动，并与自建 PLC 协同，不能重新引入持续延迟。' }
