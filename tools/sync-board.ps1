...
$Tasks = @(
    ...
    @{ T = '[M1] Android PIN 卡有时不显示（已进入配对分支但屏幕无 PIN 卡）'; M = 'M1'; P = 'P1'; S = 'In Progress'; B = '2026-09-16：代码修复及自动回归完成，待新 APK 真机复测（本轮 adb 无设备）。FFI 不再靠广播事件缓存 PIN，直接读取当前 Engine 的会话快照；控制帧写出前同步更新，成功/断开/锁定/到期清除，错误重试不续期。修复同身份重连时旧任务误删新会话；握手阶段显示真实对端名。新增 3 项内核回归：真实广播 Lagged、60 s 边界、重试与锁定、会话隔离；1 项真 QUIC/FFI 回归：断开、重连、成功、引擎重启及发起端提交。旧 FFI 对照在断开清 PIN 处失败，修复版通过。fmt、全 workspace Clippy、内核/桌面测试及 Android 69 项 JVM 测试通过；双 ABI Debug/Release APK 已构建并核对内核，Release V2 签名通过。Release 因旧文件占用改在 android/app/build/pin-validation/outputs/apk/release/app-release.apk。详见 docs/14-pairing-state.md。' }
    @{ T = '[M1] 引擎停止后立即重启：QUIC 端口尚未释放'; M = 'M1'; P = 'P1'; S = 'Todo'; B = '2026-09-16 真 QUIC/FFI 配对回归复现：存在未完成配对的连接时，engineStop 返回后立刻以相同端口 engineStart，Windows 报 1008 / UDP 绑定失败 / os error 10048。当前停止路径只发出关闭信号并等待 accept 循环，尚未保证 QUIC 后台资源释放。复现：core/crates/audiolink-ffi/tests/pairing_lifecycle.rs 第二次启动前去掉 config.listen_port = free_port()。当前 PIN 回归使用新端口隔离该问题，不作为同端口重启可用的证据。验收：同端口连续启停（含配对中/已连接/推流中）成功，停止后没有遗留会话或监听；明确取消及并发启停语义。详见 docs/14-pairing-state.md。' }
