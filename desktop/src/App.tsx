 ...
 * **当时不做**（M1 范围）：托盘、开机自启、双语、设备自动发现列表、曲线图 ——
 * 其中**托盘 / 开机自启 / 双语已在 M5 落地**（`src-tauri` 的 `build_tray` 与 `tauri-plugin-autostart`；
 * `i18n.ts` + i18n 护栏），所以这条只描述 `App.tsx` 自身当时的范围，别再当成全局现状读。
