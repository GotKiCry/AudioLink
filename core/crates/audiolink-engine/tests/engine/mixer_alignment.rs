...
//!
//! 已知缺口（写在这里，不让它藏在文档角落）：**两个发送端之间仍没有采样级对齐**。
//! 协议 §7 的 epoch 是「发送端指定、接收端排播」，M4 的拓扑（2 部手机 → 1 台 PC）里
//! 缺少一个平级的基准来源。**其中一半已补上**：M4 落地了「接收端广播 epoch」
//! （`docs/39`：`Engine::broadcast_epoch()` 与 `Engine::stream_axes()` 读数）；**仍未做**的是
//! 「发送端按该基准预约发送」与逐源采样级对齐（`docs/38` / `docs/39` §6.2）。
...
async fn two_aligned_sources_keep_feeding_the_mixer() {
    ...
    let stats = outcome.expect("90 s 内必须完成连接与开流");
