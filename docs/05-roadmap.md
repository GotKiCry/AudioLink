...
| M2 | 稳定性与质量 | L（2–3 周） | 弱网（5 Mbps/2% 丢包/30 ms 抖动）可听；8 h soak 无故障 |
...
⚠️ **关键路径风险**：Windows 采集事件驱动缓冲能否稳定压到 10–20 ms（部分驱动/独占设备例外）；Opus 编解码延迟实测值可能与理论值有偏差 → **在 M1 第一周就用 `latency-probe` 实测各环节分解，若某一环超标，先解决它再往下走**。
...
2. **丢包对策**：Opus in-band FEC + PLC +（可选）NACK 重传；
...
| 项 | 标准 | 方法 |
|---|---|---|
| 抗抖动 | 抖动 30 ms 无爆音 | netem 注入 |
| 抗丢包 | 2% 随机丢包下可懂、无长断音 | netem 注入 |
| 自适应 | 丢包 > 1% 时码率下降并可见 | 遥测时间线 |
| 长时 | 8 h 无崩溃/无静音/无漂移 | soak 脚本 + 遥测 |
...
5. 合规清单：`LICENSE` / `NOTICE` / 隐私说明（不采集音频内容、不上传遥测）。
