...
6. ~~**配对**：PIN 流程最小可用（白名单可先落盘为明文 JSON）~~ —— **已随本轮移除**，见 `docs/72-remove-pairing.md`。
...
| 抗丢包 | 2% 随机丢包下可懂、无长断音（**静音总占比可判**；~~「连续静音」推不出来，属遗留~~ → **连续性已可量**（2026-09-18，`docs/57`）；**判据阈值待产品口径**） | netem 注入 |
...
5. 合规清单：`LICENSE` / `NOTICE` / 隐私说明（音频只在你连上的设备之间直连传输、不上传第三方）。
   - **落点：`docs/privacy.md`**（2026-09-17 补）。该文逐条给出代码/清单出处（采集范围、音频去向、本地落盘、有无上报、权限逐条、如何撤回），并记录待办：本行「不采集音频内容」的措辞与产品核心功能矛盾（「取消配对」那条已随本轮移除认证而消解，见 `docs/72-remove-pairing.md`）—— 见该文 §7。
