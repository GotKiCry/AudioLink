...
**当前实现（2026-09-16）**：发送端先发当前主包，再发送上一帧的副本，因此同一序号的两个副本至少相隔一个 20 ms 帧时距；副本保持 `ptype=AUDIO` 并设置 `FEC_REDUNDANT`。接收端在有界乱序窗内按序号去重，谁先到就采用谁，主包丢失时上一帧副本仍可在重排截止前补回。链路码率统计两个数据报，约为 320 kbps；丢包分母、PLC、迟到和到达抖动只按唯一序号计算，避免冗余本身污染质量指标。NACK 尚未实现，继续作为 M2 下一项。
