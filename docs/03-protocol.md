...
**当前实现（2026-09-16）**：发送端先发当前主包，再发送上一帧的副本，因此同一序号的两个副本至少相隔一个 20 ms 帧时距；副本保持 `ptype=AUDIO` 并设置 `FEC_REDUNDANT`。接收端在有界乱序窗内按序号去重，谁先到就采用谁，主包丢失时上一帧副本仍可在重排截止前补回。链路码率统计两个数据报，约为 320 kbps；丢包分母、PLC、迟到和到达抖动只按唯一序号计算，避免冗余本身污染质量指标。**NACK 已于同日落地（2026-09-16）**：接收侧从序号洞里认出缺哪几帧（重试 ≤ 5 次 / 间隔 10 ms / 窗口 1 s），仅在 QUIC 平滑 RTT < 30 ms 时发送；发送侧保留最近 1 s 的已编码帧按序号重发（序号与 `sample_index` 原样保留，且**不计入本机发送侧分母**）；重排窗在「有洞且播放队列有余量」时开启有界等待 `min(2×RTT + 5 ms, 30 ms)`，到点即按原策略交付。真实 QUIC + 成对丢包（每 10 个丢连续 2 个 ≈ 20%）实测：接收侧 `plc_count = 0`；把 RTT 门控关掉的对照实验为 16（= 丢包数）。详见 `docs/21-m2-nack-retransmit.md`。
...
| `proto` | TXT + JSON | **带 `0x` 前缀的 4 位小写 hex 字符串**，如 `"0x0201"`（解析时大小写均可） | ALP 版本（`0x0201`）；不匹配**不丢弃**，UI 标记版本不兼容并禁止从列表连接（与当前握手的精确版本匹配一致） |
