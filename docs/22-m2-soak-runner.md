...
- ~~**真正的 8 h 长跑**尚未执行（工具就绪；跑一次 8 h 是环境时间成本，不是代码问题）~~
  → **口径更新（task-43，2026-09-17）：已在跑，不是「未执行」**。进程 `soak-runner.exe.locked-8h`（PID 60896）
  起跑于 **2026-09-17 13:05:48**，日志 `target/evidence/soak/soak-8h-head.log`；截至 09-17 18:37 已到
  `t=19905 s` / 28800 s（约 **69%**），读数 `丢包 0 / 异常 0 / NACK 90`。
  ⚠️ 读时差异：日志末次落盘（18:07:51）比读数时刻落后约 30 min，属刷盘延迟，**以 `t=` 计数为准**。
  ⚠️ 产物在 `target/`（gitignored），跑完后报告必须落进 `docs/**`，否则这次长跑等于没证据。
...
### 11.5 ~~遗留~~：「长断音」（连续静音）~~推不出来~~ → **已落地**（2026-09-18，见 `docs/57-m2-long-silence.md`）
...
真正的连续性指标需要在引擎播放线程里直接统计「最长连续静音拍数」，并经**事件**（不是 `StreamStats`）上报 —— **已实现**：

> **2026-09-18 更新（`docs/57-m2-long-silence.md`）**：`EngineEvent::SilenceDetected { peer, segment }` +
> `Engine::silence_report(peer)` 已落地（`silence.rs` 纯逻辑 + 播放线程 5 处写帧点 + 关流结算）。
> 上面那条**下界**（`summary.longest_silence_lower_bound_beats`）**保留**：它仍然正确，只是不再是唯一能给的数 ——
> 它同时也是「真值接进报告」这一步的对照（本轮**未做**，见 `docs/57` §6 第 1 项）。
