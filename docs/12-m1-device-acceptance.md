...
| 30 min 无断流 | ⏳ 见 §5（在 task-8 的最终固件上跑） | — |
...
| 12 | soak 中「已封口 4096 帧」看起来像**发送端停发** | `device-link` device 模式的该显示值 = `MeasurementTap::sealed_orphans`，而 tap 容量固定 **4096**（`device_link.rs:844`），满了丢最旧 ⇒ **计数饱和**，不是停发（旁证：同期链路 loss 0.00%、对端队列仍在变化） | 📝 工具显示缺陷：device 模式应换成无上限的朴素计数（不影响任何验收数字） |
...
2. **30 min soak**：在当前固件上跑（记录 underruns / late_drops / loss / 断流）；
...
| 迟到丢弃 `late_drops` | 0 → **53**（前 ~5 min 内涨到 43，之后基本平） | ❌ 出现过丢弃 |
| 供给欠载 `underruns` | 7 → **69**（约 +2.7/min 持续增长） | ⚠️ 未收敛 |
...
> 证据：`acceptance/soak30.log`（1800 行逐秒）、`acceptance/soak30.json`（账本）、`acceptance/soak30-counters.txt`（前后设备侧计数）。
...
> 三轮复核共 53 个溯源项；**每一条都在本文件留下修订痕迹**。主结论三次均未被推翻。
