...
> ## ⚠️ 落地状态（后续轮次补记，**读本文前必读**）
>
> **本文是修复前的静态审计快照**（行号基准 HEAD cd4d6dc，runtime.rs 4182 行；该文件现为 4809 行）。下列结论**已被实现取代**，不要当现状缺口来读：
>
> - **§0 结论 1/2、§2.2 的 owner 生命周期缺口：一半已修，一半仍未修（第 104 轮实测，task-21）。**
>   - **已修：新会话建立播放管线时的接管（§4.2 判据 ⑧）。** PlayoutMixSlot.owner 用**源号世代**（NEXT_MIX_SOURCE 单调分配）+ compare_exchange(0, source) 实现「owner 可让位、新会话可接管」，即本文 §3.1 方案 1 的落地；且比方案 1 的裸 AtomicBool 更严 —— 裸布尔存在 ABA：旧 owner 迟到的退出会把刚接管的新 owner 的标志清成 false，第三路随后就能抢到 owner，同一台设备被两路同时写。实测：接收端会话轮换 3 次，播放设备打开次数 **1 → 2 → 3**，每轮让位后活跃混音源 Some(1) 且源号不泄漏。
>   - **未修：§3.1 结尾那句「顺带修好的：M4 里『接收端 owner 会话先断、其他路还在』的静音缺陷」并没有落地。** 第 104 轮 task-21 的 other_sources_keep_playing_after_the_owner_leaves 稳定红（连跑 3 次逐字节一致）：两路混音对照 RMS 0.2504，owner 退出后仅剩那一路 RMS **精确 0.0000**，播放设备打开次数**停在 1**，而该路**仍在被接收 320888 bps**、活跃混音源 Some(1)。因果链闭合：帧在进来、在混音，**但没有任何线程写设备**。
>   - 根因（静态，已复核）：① PlayoutHandle::drop 的让位只做 compare_exchange(本路源号 → 0) —— **只清位，不找人接管**；② acquire_playout_mixer 的 CAS 只在 spawn_playout_thread（收到 OPEN_STREAM 时）被**调用一次**，**已经在混音的 guest 没有任何通路重新走它**；③ spawn_playout_thread 里 factory 按 is_owner 裁剪，guest 拿不到 factory；④ guest 的 write_frame 在 sink == None 时直接 return FrameWrite::Ok，既不建 sink、也不上报「设备无人持有」。
>   - 修法方向：**懒接管** —— 所有会话都持 factory 与槽位，播放循环发现 sink == None 且 owner == 0 时 CAS 抢 owner 并打开设备（CAS 已保证单赢家），打开失败则让回并退避。自愈式，不依赖跨会话通知。
