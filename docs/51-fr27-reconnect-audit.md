...
>
> **本文是修复前的静态审计快照**（行号基准 HEAD `cd4d6dc`，`runtime.rs` 4182 行）。下列结论**已被实现取代**，不要当现状缺口来读：
>
> - **§0 结论 1/2、§2.2、§3.1 的 owner 生命周期缺口已修复**。现状：`PlayoutMixSlot.owner: AtomicU32` 用**源号世代**（`NEXT_MIX_SOURCE` 单调分配）+ `compare_exchange(0, source)` 实现「owner 可让位、新会话可接管」，这正是本文 §3.1 方案 1 的落地；且比方案 1 的裸 `AtomicBool` 更严 —— 裸布尔存在 ABA：旧 owner 迟到的退出会把刚接管的新 owner 的标志清成 false，第三路随后就能抢到 owner，同一台设备被两路同时写。修复同时覆盖了 §3.1 结尾预言的「M4 里接收端 owner 会话先断、其他路还在」的静音缺陷。
> - **§4.2 判据 ⑧（播放设备被重新打开）与 §7 第 189 行要求的回归测试（接收端两个发送会话、先建的 owner 断开、另一路仍出声）在修复落地时并未补齐** —— `tests/engine/` 全目录 grep `is_owner|owner_slot|factory` 零命中，等于这条修复长期**没有测试守着**。补位于 `task-21` / `tests/engine/mixer_owner_handover.rs`。
> - **本文的行号基准已过期**：`runtime.rs` 现为 4809 行。本文所有 `runtime.rs:NNNN` 引用一律按**函数名**定位，**不要按行号**。
>
> **教训（值得记住）**：一份「修复前的诊断文档」若不在顶部标注落地状态，后续轮次的审计者会把它当成**现状缺口**照抄 —— 这已经真实发生过一次：有轮次据 §2.2 发出了「接收端 8 路压测会出现『表里有 8 路、设备只开一次后永久静音』假绿」的警告，而该缺口其实早已修好。
