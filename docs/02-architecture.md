...
**2026-09-23 补充（稳态水位结算）**：档位不变的稳态下，水位持续超出目标一整帧摆幅达 500 ms 时，播放层丢掉最旧一帧（游标正常推进、记 `depth_drops`、走既有 2.5 ms 边界平滑）。它补上此前「稳态无回落分支」的缺口：发送时钟、本机播放节拍、设备音频时钟三个时钟域的 ppm 级速率残差，不再以「爬满队列 → 投递失败 → 欠载 → 120 ms 淡出掩盖」的失控链释放（docs/12 §11 的 30 min 水位 43 → 276 ms），而是换成有界的内容跳过，盈余存在多久结算就发生多久。与被移除的两版水位护栏（docs/12 §11.12/§11.14，代价 7×/103×）的区别：持续窗口过滤突发、每次只结算一帧、目标之上永远保留一整帧抖动摆幅。发送侧顺带补齐：会话编码线程的 2 ms 轮询在 Windows 下同播放线程一样持有 1 ms 定时器粒度，消除 15.6 ms 粒度造成的整批编码、成簇发送。
