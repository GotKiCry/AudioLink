...
Android 接收入口和桌面「收听其他设备的声音」显示「局域网主机」列表，并提供「刷新主机」按钮。刷新会清空旧结果、重新建立 UDP 浏览器；搜索期间按钮禁用，失败后可再次刷新。条目包含名称与地址，点击后**直接建立连接**（没有配对码这一步）。手动地址输入保留。Android 的扫描不依赖音频服务启动，只在接收入口可见且应用处于前台时运行；取消收集时停止 Rust 浏览器并释放 Wi-Fi 接收锁。
...
- 内核 `PeerStatus.initiated_locally` 记录每条真实会话的角色，并映射到桌面 `PeerView.receiving`。收听主机时不触发自动推流；入站设备**连接完成后即自动推流**（本版本没有信任筛选，任何接入的设备都按同一规则处理）。角色来自会话本身，不靠地址缓存推断。

发现报文未经认证，只提供候选地址；连接时仍走完整的 QUIC/TLS 握手 —— 证书身份用于区分设备，**不做信任裁决**。无音频、密钥或任何凭据进入发现报文。当前接通 IPv4 UDP 广播，mDNS / 单播查询仍是预留方案；手动 IPv6 地址连接保持可用。
