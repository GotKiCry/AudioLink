...
**AudioLink 没有服务器、没有账号、没有云端。** 声音只在你主动配对的设备之间**点对点直连**传输，
...
| 证书是**本机自签**，不是任何 CA 签发；身份 = **证书指纹**（`SHA-256(证书 DER)` 前 8 字节 = `NodeId`） | `core/crates/audiolink-net/src/tls.rs:35-38`、`docs/03-protocol.md:364` |
| TLS 层**只负责加密**；"对端是不是你要的那台设备"由**引擎的信任库 + 6 位 PIN 配对**判定 | `core/crates/audiolink-net/src/tls.rs:88`、`:176-184`（注释：「指纹比对在 engine（§5 的信任库），这里只把证书收下来」）；信任库落盘见 §3 |
| 配对用 **6 位 PIN**：60 s 有效、失败 5 次锁定 5 分钟、PIN 走加密通道提交并绑定双方指纹 | `docs/03-protocol.md:224`（另有握手时序图 `:214`）|
...
| 本机监听 `0.0.0.0:58290`（默认全网卡）：**同一局域网的任何设备都能发起 TLS 握手**，但要收/发音频必须通过 §5 的信任判定与 PIN 配对 | `core/crates/audiolink-engine/src/runtime.rs:206,252`（默认监听）、`core/crates/audiolink-types/src/lib.rs:30`（端口常量）、`docs/03-protocol.md` §5 |
...
| 类别 | 事实 | 证据 |
|---|---|---|
| 链路遥测（RTT / 丢包 / 抖动 / 缓冲水位 / 欠载）| **只在本机界面显示**；另外两端引擎之间以 1 Hz 互发 `STREAM_STATS`（控制帧），那是**与你配对的对端**交换，不是发给第三方 | `docs/03-protocol.md` §10（定义）；`runtime.rs` 会话循环的 1 Hz ticker；面板 `docs/25-m2-telemetry-panel.md` |
...
1. `docs/manual/user-guide.zh-CN.md:13`「只在局域网内工作：不经过任何服务器，不上传音频内容，不采集遥测到云端」——
   ...
   「音频不发给任何第三方，只发给你配对的设备」（音频当然会被本机采集，否则功能不成立）。

**需要改的地方（本文只记录，不改上游文档）**：

| # | 位置 | 问题 | 建议 |
|---|---|---|---|
| 1 | `docs/05-roadmap.md` §M5 交付物 5 | 「隐私说明（**不采集音频内容**、不上传遥测）」——**前半句与产品核心功能矛盾**：本产品的功能就是采集音频并定向推送。"不采集"若被当真，等于说功能不存在 | 改为「音频只在你配对的设备之间直连传输，不上传第三方」 |
...
| 3 | 产品侧 | **取消配对没有界面出口**（§6）——用户没法在应用里"忘掉一台设备"，只能删文件 | 需要一个「移除设备 / 取消信任」的入口（FR-17 配对的对偶操作）|
...
with TLS 1.3 and mutual certificates to the devices you paired. Nothing is relayed through any
...
key, the trust store (paired peers) and UI preferences; Android declares `allowBackup="false"`.
...
implemented but **not yet device-validated**, and **unpairing has no UI** yet (see §6/§7).
