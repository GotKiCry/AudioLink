...
| `App.tsx` 的「不做：托盘、开机自启、双语」是否过时？ | **已不过时**：当前 HEAD 那段已自我更正为「当时不做（M1 范围）… 其中托盘 / 开机自启 / 双语已在 M5 落地」 |
...
| 项 | 结果 |
|---|---|
| `App.tsx` 的过时注释 | **已过时→已修好**：当前 HEAD 写的是「**当时不做**（M1 范围）… 其中**托盘 / 开机自启 / 双语已在 M5 落地**…别再当成全局现状读」——自我更正版，**不再算错** |
...
| 剩余项承接 | 真机安装验收 → 面板 `[环境] MIUI 上 adb install release APK 失败 -99`（Todo）✔；代码签名证书 → `[M5] Windows 代码签名证书`（Todo）✔；真实更新流程验证 → `[M5] 真实更新流程验证`（Todo）✔ |
...
2. **真机/外部项确实都有独立条目承接**：MIUI 安装（`[环境]` Todo）、代码签名证书（Todo）、真实更新流程验证（Todo）；
3. **`App.tsx` 那条过时注释已经修好**（你正在改的那处已是自我更正版）。
