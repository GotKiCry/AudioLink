...
>
> **2026-09-18 更新**：§0 阻塞清单与 §1.1 / §3 / §6 已就地同步真机实测 —— ①「手机与 PC 不同子网」**已解除**；
> ② §3.1 的低延迟 / 零重采样判据已在 PHK110 上复测过一轮（读数与两条口径补注见 `docs/12` §10.8 / §10.2 / §10.6）；
> ③ §3.2 补上声学量具的一键入口（`tools/acoustic-latency.ps1` + `docs/60`）。**其余步骤仍未在真机上跑过**，下面第 10 行的声明仍然有效。
...
|---|---|---|---|
| 1 | **[M1] 验收**：声学端到端 P50 ≤ 110 ms / P95 ≤ 150 ms | §2 | **方法齐备、命令齐备，但声学链路仍未验证**；需要一支麦克风 + 现场标定。零重采样 / 低延迟模式两项另有读数法（§2.11、§3）。**设备侧 A/B 已于 2026-09-18 真机复测**（PHK110，`docs/12` §10）：账本口径 e2e 推算四段 **172–182 ms**，离 110 ms 线仍差 **62–72 ms**；声学 P50/P95 当前阻塞于「本机无物理麦克风」（§3.2） |
| 2 | **[M1] 真机验收**：Android 低延迟 / 出声延迟 / 30 min 无断流 | §3 | **可执行**（30 min 有现成脚本）；「出声延迟」= §3.2 的**一键入口** `tools/acoustic-latency.ps1`（`-SelfTest` / `-DryRun` / `-Run`，用法与自检证据见 `docs/60`）；低延迟模式已于 2026-09-18 复测（`docs/12` §10.8） |
...
**当前硬阻塞**（详见 §6）：~~手机与 PC **不同子网**~~（**已解除 2026-09-18**：实测手机 `192.168.3.75/24`、PC `192.168.3.200/24` 同子网）、`adb forward` **仅 TCP**。另有本机侧阻塞：**声学出声延迟缺物理麦克风**（§3.2）。~~**Android 无发送入口**（本手册新增的第 4 条）~~ → **已解除**：`6f04cbf` 落了连接/推流入口（2026-09-17 复核，见 §5.2）。**MIUI `adb install` `-99` 已移出本清单**（2026-09-18 用户裁决：MIUI/HyperOS 的系统侧限制，非本项目要解决的问题；看板条目同步移除，处置见 `docs/manual/troubleshooting.zh-CN.md`）。
...
| **同一子网** | 手机与 PC 必须在**同一网段**（前三段相同） | ~~🔴 现成阻塞：手机 `192.168.31.231/24` vs PC `192.168.3.200/24` → **不可达**（跨网段既发现不到、直连也不通，见 `docs/manual/troubleshooting.zh-CN.md`）~~ → **已解除（2026-09-18）**：实测手机 **`192.168.3.75/24`** 与 PC **`192.168.3.200/24`** **同子网**，同子网直连推流跑通（出处 `target/evidence/m1-device-p1/env-facts.txt`；复测见 `docs/12` §10）。处置照旧：换网络后先确认前三段一致再 `ping` |
...
> **不作待办跟踪**：MIUI/HyperOS 上 `adb install` release 包报 **`Failure [-99]`** 是**系统侧限制**（开发者选项打开「USB 安装」即可；debug 包不受影响），2026-09-18 按用户裁决移出看板与本文清单 —— 它不是本项目要解决的问题。真撞上时的排错见 `docs/manual/troubleshooting.zh-CN.md`。
...
1. **手机侧播放链吃满**：AudioTrack 实际缓冲 80.08 ms（请求 960 帧被顶到 3844 帧）+ flinger track latency 101 ms —— 这是 `docs/12` §2.1b 的实测，且 `§4 #9` 指出「内核 → Kotlin PCM 推送 ≈ 2× 实时」会让延迟搬进内核队列；**先看这两处**，别急着怀疑网络。⚠️ **口径更新（2026-09-18）**：`§4 #9` 的「≈ 2× 实时」根因已在 `docs/12` §9 修复（供给回到 ≈ 1× 实时），但「延迟搬进内核队列」这个**现象仍然成立** —— 修复后实测水位 40 → 94–100 ms、与 flinger 省下的 60 ms 一比一抵消（`docs/12` §10.5）。
2. **手机待播队列水位**：⚠️ **口径更新（2026-09-18）**：~~30 min 档实测 P50 200 ms / 顶到 320 ms 上限（`docs/12` §5.1）~~ 那是**修复前**那轮 30 min 的读数（队列缺陷与 PCM 长度缺陷都还在）；修复后同项实测 **40–100 ms**（`docs/12` §10.6，四段 96 s 窗口）—— 修复后的 30 min 结论仍待补。口径不变：水位不降下来，任何链路优化都填不满 110 ms。
...
> ⚠️ **两条口径补注（2026-09-18 真机实测，读数见 `docs/12` §10.2 / §10.6 / §10.8）**：
> 1. `FrmCnt 3844` 只在**容量未收缩**时成立；点了 UI 的「容量收缩」后 `FrmCnt` 变成 `960r`（`docs/12` §10.2 的 D / C 段）；
> 2. 这里的 `Underruns 0` 是 **track 口径**（flinger 该 track 的列）与系统欠载口径，**不代表**对端账本里的内核播放环欠载：
>    满灌档实测 **0.719 次/s**、30 ms 档 **0.104 次/s**。三个「欠载」不是同一个计数器，不可互相抵消（定义见 `docs/12` §10.1）。
...
# ② 预演：打印步骤、现场清单、误差表；对麦克风只做 3 s 只读电平探测
pwsh tools/acoustic-latency.ps1 -DryRun -MicDevice "<麦克风 dshow 名>" -Peer <手机IP>

# ③ 正式测量（现场：插好麦克风 + 手机处于同一子网并已开始接收）
pwsh tools/acoustic-latency.ps1 -Run -MicDevice "<麦克风 dshow 名>" -Peer <手机IP> -ExtraPathCm <声程差 cm>
```

量具本体 `tools/acoustic-latency.ps1`（另有 `-AnalyseWav` 可离线分析一段已有录音）。用法 / 参数速查 / 摆位清单 / 误差表 / 退出码（0 达标 · 1 不达标 · 3 检测失败 · 4 设备不可用）、以及自检 14/14 的证据见 `docs/60-p1-acceptance-notes.md`。判读阈值不变（§2.8）：**P50 ≤ 110 ms 且 P95 ≤ 150 ms 且样本 ≥ 6 对**。

> ⚠️ **当前阻塞（2026-09-18 核实，未解除）**：本机 dshow 音频**输入**端点只有两个**虚拟**设备
> （`Virtual Mic (Virtual Mic for AudioRelay)`、`麦克风阵列 (网易虚拟音频设备)`），对后者做 3 s 只读电平探测得
> `mean_volume = -91.0 dB / max_volume = -90.3 dB` ⇒ 判为拿不到声学信号。
> **没有物理麦克风就没有数字**：脚本按纪律以**退出码 4** 失败，所有延迟字段为 `null`（不输出伪造数字）。
> 解除路径：插一支物理麦克风 → `-DryRun` 应先看到「✅ 该端点能拿到声学信号」，再跑 `-Run`（`docs/60` §8）。
...
> 原第 3 条（MIUI `adb install` release 包 `Failure [-99]`）已于 2026-09-18 按用户裁决**移出跟踪**：那是 MIUI/HyperOS 的系统侧限制，不是本项目待办。

| # | 阻塞 | 现象 | 解除路径 | 出处 |
|---|---|---|---|---|
| ~~1~~ | ~~**手机与 PC 不同子网**~~ → **已解除（2026-09-18）** | ~~手机 `192.168.31.231/24` vs PC `192.168.3.200/24` → 发现不到、直连不通~~ → 实测手机 **`192.168.3.75/24`**、PC **`192.168.3.200/24`**，**同子网**；同子网直连推流跑通（ab2 四段） | 前提不变：本项目仍需手机与 PC 同网段；换网络后先确认前三段一致再 `ping` | ~~看板 `[M3] 真机验收` 条目~~ → `target/evidence/m1-device-p1/env-facts.txt`（PHK110 / adb-tls 无线调试 / 装机 APK 与设备拉回包**同 hash**）；复测见 `docs/12` §10 |
