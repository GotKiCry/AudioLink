...
**当前硬阻塞**（详见 §6）：~~手机与 PC **不同子网**~~（**已解除 2026-09-18**：实测手机 `192.168.3.75/24`、PC `192.168.3.200/24` 同子网）、`adb forward` **仅 TCP**、MIUI `adb install` **-99**。另有本机侧阻塞：**声学出声延迟缺物理麦克风**（§3.2）。~~**Android 无发送入口**（本手册新增的第 4 条）~~ → **已解除**：`6f04cbf` 落了连接/推流入口（2026-09-17 复核，见 §5.2）。
