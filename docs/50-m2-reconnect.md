...
- 不变量：§5 握手（**认证阶段已随本轮移除**，见 `docs/72-remove-pairing.md`）、48 kHz 全链路、`Failed` 可重启、任何失败都有明确去向（`docs/02-architecture.md` §11、架构 §4）。
...
- `connect_inner`（`runtime.rs:1230`）是唯一的拨号入口，**必须**走完整的 §5 握手：`Handshake::new`（`1499-1502`）→ `handshake.on_control`（`1551`）。任何「重连只连 QUIC 不握手」的捷径都会让对端身份失去校验（§3.4）。
...
### 3.4 安全性要求：重连不得绕过握手与身份校验

> ⚠️ **本轮更新（去配对）**：PIN / 信任库 / `is_trusted` 已整体移除（见 `docs/72-remove-pairing.md`）。
> 下面第 1 条里的 `is_trusted` / `trusted` 传参、以及第 3、4、5 条（未信任不得重拨、PIN 语义、信任库写入）所依赖的机制
> **已不存在**，只作当时的记账保留；**仍然生效**的是：重拨必须走完整 §5 握手（第 1 条的主体）、身份必须一致（第 2 条）、
> 身份不匹配的尝试不得污染会话表（第 6 条）。
...
- 已有护栏要保留并在文档里点名：旧任务退出时必须 `Arc::ptr_eq` 命中自己才摘表项（`2215-2221`，缘由见 `docs/15-engine-restart.md`）。
...
- [ ] 重连参数按 3 s 预算反推（§2.7）
- [ ] 重连不改变握手与身份校验语义（§3.4：仍然生效的第 1 / 2 / 6 条逐条补测试；其余三条的机制已移除）
